...
if [[ -f "${HOME}/.config/.configs/.bash.py" ]]; then
  echo "❌  当前环境无需安装任何依赖 / The current environment does not require any dependencies to be installed!"
  exit 1
fi
...
main() {
  ...
  ok "🎉 所需依赖已安装 / The required dependencies have already been installed! ✨ 🌟 ✨"
  suggest_reload
}
